function [test_se] = RBF_train(spread,mn,trainnumber,testnumber)
    ...
    for i =1:size(Capacity_train_temp,1)
        Capacity_train(:,i) = Capacity_train_temp(i,:);
    end

    net=newrb(Battery_data_train,Capacity_train,5e-5,spread,mn,1);
    ...
    test_se = sum(test_error.*test_error);%最小二乘法    
end
