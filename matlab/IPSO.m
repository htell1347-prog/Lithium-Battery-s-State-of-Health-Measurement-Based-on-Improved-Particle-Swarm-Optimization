function [spreadbest,mnbest,gbestsum,pbestfitnessaveragesum] = IPSO(spreadmin,spreadmax,mnmin,mnmax,trainnumber,testnumber)
...
%     v=vstart;
    ...
    [gbestfitness,m] = min(pbestfitness);%gbestfitness为全局最优粒子对应的适应值，群体极值适应度
    gbest=pbest(m,:); %gbest为全局最优粒子的位置，群体极值   
    ...
    for j=1:M
        ...
        gbest=pbest(m,:);
        pbestfitnessaveragesum(j,:) = mean(pbestfitness);
        gbestsum(j,1) = gbest(1);
        gbestsum(j,2) = gbest(2);
        gbestsum(j,3) = gbestfitness;       
    end
    spreadbest=gbest(1);
    mnbest=gbest(2);
